...
public class NHBankSQL {
	...
	public int acCount() {
		...
		String sql = "SELECT COUNT(*) FROM NHBANK";
	...
	public void insert(NHClient nhc) {
		String sql = "INSERT INTO NHBANK VALUES(?,?,?,?)";

		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, nhc.getAccountNo());
			pstmt.setInt(2, nhc.getClientNo());
			pstmt.setString(3, nhc.getcName());
			pstmt.setInt(4, nhc.getBalance());

			int result = pstmt.executeUpdate();

			if (result > 0) {
				System.out.println("님 계좌가 생성됐습니다.");
			} else {
	...
	public void deposit(String accountNo, int dbalance) {

		String sql = "UPDATE NHBANK SET BALANCE = BALANCE+? WHERE ACCOUNTNO =?";

		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, dbalance);
			pstmt.setString(2, accountNo);

			int result = pstmt.executeUpdate();

			if (result > 0) {
				System.out.println(dbalance + "원 입금 성공!");
			} else {
	...
	public void withdraw(String accountNo, int wbalance) {
		String sql = "UPDATE NHBANK SET BALANCE = BALANCE-? WHERE ACCOUNTNO=?";

		try {
			pstmt = con.prepareStatement(sql);

			pstmt.setInt(1, wbalance);
			pstmt.setString(2, accountNo);

			int result = pstmt.executeUpdate();

			if (result > 0) {
				System.out.println(wbalance + "원 출금 성공!");
			} else {
	...
	public int checkBalance(String accountNo) {
		...
		String sql = "SELECT BALANCE FROM NHBANK WHERE ACCOUNTNO =?";
	...
	public boolean sendCheck(String sAccount) {
		...
		String sql = "SELECT ACCOUNTNO FROM NHBANK WHERE ACCOUNTNO=?";
